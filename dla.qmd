...
Open vocabulary analysis does not require two distinct groups. For example, we may want to investigate the differences in language associated with openness to experience. A simple way to do this is to measuring the correlation between word frequencies in documents and the documents' authors' `openness` score. 
...
## Generating Dictionaries With Open Vocabulary Analysis
...
Can we use this new surprise dictionary to test our hypothesis that true autobiographical stories include more surprise than imagined stories? There is no guarantee that surprise words in Tweets will be the same as surprise words in autobiographical stories. With this in mind, we can proceed cautiously:
