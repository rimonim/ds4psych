...
```{r}
textstat_word2vec <- function(dfm, model){
  feats <- featnames(dfm)
...
hippocorpus_word2vec <- hippocorpus_dfm |> 
  textstat_word2vec(word2vec_mod)
...
Once again we found a significant difference between recalled and imagined stories (p < .001), but this time in the opposite direction---recalled stories have _less_ surprise-related language! We also found a significant difference between retold and imagined stories (p < .001) such that retold stories have less surprise-related language. These findings seem to go against those of @sap_etal_2022, but stick around for @sec-navigating-vectorspace to learn why this might be misleading.
...
-   **Efficient for Large Datasets:** This means that models can be trained on enormous amounts of text. Multiple such models are available for download on the Internet.
:::
